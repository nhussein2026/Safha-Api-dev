...
var store = async (req, res, next) => {
    ...
    console.log('reqqqqqqqqqqqqqqqqqqqqqq', req?.file)
    ...
    try {
        ...
        if (created) {
            ...
            // response.data = bookTransformer(newBook)
            response.data = newBook
        } else {
            response.success = false
...
var index = async function (req, res, next) {
    ...
    try {
        ...
        if (Array?.isArray(books)) {
            // response.data = bookTransformer(books)
            response.data = books
        } else {
            res.status(404)
            res.success = false
...
var show = async function (req, res, next) {
    ...
    // var book = await models?.User?.findByPk(id,{
    //     include: [
    //         {
    //             model: models?.Book,
    //         },
...
var destroy = async function (req, res, next) {
    ...
    console.log(fn("now"));
    // new Date()
    var deleted = await models?.Book?.update({
        deletedAt: fn("now"),
    },
    {
        where:{
